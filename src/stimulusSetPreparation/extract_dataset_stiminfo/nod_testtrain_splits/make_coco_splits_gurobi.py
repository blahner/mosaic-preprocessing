...
def main(args):
    ...
    assert(len(embedding_paths) == 120)
    embeddings = []
    image_filename = []
    for embedding_path in embedding_paths:
        embeddings.append(np.load(embedding_path)[0])
        filename = embedding_path.split('/')[-1]
        image_filename.append(filename.split(f'_model-{model_name}.npy'))
    ...
    col_to_idx = {label: i for i, label in enumerate(image_filename)}
    ...
    tl = 43200 #max runtime in seconds
    ...
    #save groups
    with open(os.path.join(args.dataset_root, "derivatives", "stimuli_metadata", "testtrain_split", "coco_groupings.pkl"), 'wb') as f:
