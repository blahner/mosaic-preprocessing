#!/usr/bin/env python3
"""
fMRIPrep quality-assurance movie for BOLD5000.
...
number is the anat one varies by subject (ses-16 for CSI1-3, a different
number for CSI4, which has fewer sessions overall). Rather than hardcode a
session name, find_anat_dir() discovers whichever session actually has an
anat/ subfolder for that subject. By default only the main "5000scenes" task
is included (use --tasks localizer to add localizer runs).
--max_sessions evenly subsamples down to a manageable movie length.

Usage
-----
  python bold5000_fmriprep_qc_movie.py [--tasks 5000scenes] [--max_sessions 8]
                                        [--subs sub-CSI1 sub-CSI2 ...]

Output
------
  OUT_DIR / BOLD5000_<subs>_qc_small.mp4
"""
...
FMRIPREP = Path(os.getenv("DATASETS_ROOT")) / "BOLD5000/derivatives/fmriprep"
OUT_DIR  = Path(os.getenv("PROJECT_ROOT")) / "src/fmriDatasetPreparation/visualizations/output/qc"

DEFAULT_TASKS = ['5000scenes']   # main experimental task; task-localizer is excluded by default
...
# BOLD5000 keeps anatomy in a dedicated anat-only session, but which session
# number that is varies by subject (ses-16 for CSI1-3, a different number
# for CSI4). find_anat_dir() discovers whichever session actually has an
# anat/ subfolder rather than assuming a fixed session name. Sessions,
# tasks, and runs are discovered dynamically by globbing.
