#!/usr/bin/env python3
"""
fMRIPrep quality-assurance movie for THINGS.
...
THINGS has no session-level anatomy at all — anat outputs live directly
under sub-XX/anat/ (a sibling of the ses-* folders, reused across every
session) rather than in a dedicated ses-* session, and every anat filename
carries an extra "rec-pydeface" entity (e.g.
sub-01_rec-pydeface_desc-preproc_T1w.nii.gz) since defacing was applied.
Anat files are therefore located by globbing on distinctive suffixes rather
than exact filenames. By default only the main "things" task is included
(use --tasks 6cat pRF rest to add the localizer sessions).

Usage
-----
  python things_fmriprep_qc_movie.py [--tasks things] [--max_sessions N]
                                      [--subs sub-01 sub-02 ...]

Output
------
  OUT_DIR / THINGS_<subs>_qc_small.mp4
"""
...
# ---------------------------------------------------------------------------
DATASET_NAME = "THINGS"
FMRIPREP = Path(os.getenv("DATASETS_ROOT")) / "THINGS_fmri/derivatives/fmriprep"
OUT_DIR  = Path(os.getenv("PROJECT_ROOT")) / "src/fmriDatasetPreparation/visualizations/output/qc"

DEFAULT_TASKS = ['things']   # main experimental task; task-6cat/task-pRF/task-rest
                               # localizer runs are excluded by default
...
# THINGS has no session-level anatomy: anat outputs live directly under
# sub-XX/anat/ (a sibling of the ses-* folders) rather than in a dedicated
# ses-* session, and every anat filename carries an extra "rec-pydeface"
# entity. find_anat_dir() checks for a bare sub-XX/anat/ folder first (see
# below) so this Just Works without special-casing, and find_anat_file()
# globs on distinctive suffixes rather than exact filenames so the extra
# entity doesn't matter. Sessions/tasks/runs are discovered dynamically.
...
def make_run_summary_fig(sub, sessions, tasks):
    ...
    # "things01" vs. long "perceptionNaturalImageTraining13"-style names in
    # other datasets), and stacked in rows (reserved via measured height)
    # instead of a single axes-anchored row — a wide single-row legend
    # previously forced tight_layout to squeeze the plot axes down to a
    # sliver to keep the legend on-canvas.
