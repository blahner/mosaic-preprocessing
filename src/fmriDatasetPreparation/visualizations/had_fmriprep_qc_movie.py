#!/usr/bin/env python3
"""
fMRIPrep quality-assurance movie for HumanActionsDataset.
...
HAD has only a single session per subject (ses-action01) which contains
BOTH anat/ and func/ together — unlike most other datasets here, there is no
separate anat-only session to exclude from the functional-session loop.
list_functional_sessions() includes any session with a func/ subfolder
regardless of whether it also has anat/, so this session is correctly kept
rather than accidentally excluded.

Usage
-----
  python had_fmriprep_qc_movie.py [--tasks action] [--subs sub-01 sub-02 ...]

Output
------
  OUT_DIR / HumanActionsDataset_<subs>_qc_small.mp4
"""
...
# ---------------------------------------------------------------------------
DATASET_NAME = "HumanActionsDataset"
FMRIPREP = Path(os.getenv("DATASETS_ROOT")) / "HumanActionsDataset/derivatives/fmriprep"
OUT_DIR  = Path(os.getenv("PROJECT_ROOT")) / "src/fmriDatasetPreparation/visualizations/output/qc"

DEFAULT_TASKS = ['action']   # the only task in this dataset
...
# HAD has only one session per subject (ses-action01), and anat/ and func/
# coexist in that same session — there is no separate anat-only session to
# exclude. list_functional_sessions() includes any session with a func/
# subfolder regardless of whether it also has anat/, so the merged session
# is correctly kept rather than accidentally treated as anat-only and
# skipped.
