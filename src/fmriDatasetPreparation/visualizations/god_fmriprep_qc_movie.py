#!/usr/bin/env python3
"""
fMRIPrep quality-assurance movie for GenericObjectDecoding.
...
GOD's session names are semantic strings, not zero-padded numbers
(ses-perceptionTraining01, ses-perceptionTest01, ses-imageryTest01, ...),
and run counts differ by session type (perception=10 runs, imagery=8 runs).
...
Usage
-----
  python god_fmriprep_qc_movie.py [--tasks perception] [--max_sessions 8]
                                   [--subs sub-01 sub-02 ...]

Output
------
  OUT_DIR / GenericObjectDecoding_<subs>_qc_small.mp4
"""
...
# ---------------------------------------------------------------------------
DATASET_NAME = "GenericObjectDecoding"
FMRIPREP = Path(os.getenv("DATASETS_ROOT")) / "GenericObjectDecoding/derivatives/fmriprep"
...
# GOD keeps anatomy in a dedicated ses-anatomy session (no func/), and mixes
# multiple task types (perception/imagery) across semantically-named
# sessions with different run counts per session type. Sessions, tasks, and
# runs are discovered by globbing instead of assumed from a fixed template.
