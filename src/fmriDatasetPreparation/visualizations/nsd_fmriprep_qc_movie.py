#!/usr/bin/env python3
"""
fMRIPrep quality-assurance movie for NaturalScenesDataset (NSD).
...
Unlike BOLDMomentsDataset, NSD subjects have up to 40 functional sessions
...
#
# NSD keeps anatomy in a session dedicated to it (ses-nsdanat, no func/) that
# is otherwise unrelated to whichever functional session is being QC'd, and
# has up to 40 functional sessions with variable run counts. Sessions, tasks,
# and runs are discovered by globbing instead of assumed from a fixed
# template, since no single (task, n_runs) pair describes every session.
# ---------------------------------------------------------------------------
