#!/usr/bin/env python3
"""
fMRIPrep quality-assurance movie for NaturalObjectDataset.
...
NOD sessions map roughly 1:1 to task (ses-coco/ses-floc/ses-imagenet0X/
ses-prf), with run counts that vary by session type and subject, and the
dedicated anat session (ses-anat) carries an extra "run-01" filename entity
(e.g. sub-01_ses-anat_run-01_desc-preproc_T1w.nii.gz). Anat/func files are
therefore located by globbing on distinctive suffixes rather than exact
filenames, and sessions/tasks/runs are discovered dynamically. By default
both main stimulus-driven tasks ("coco" and "imagenet") are included (use
--tasks to add the floc/prf localizer/mapping scans too).

Usage
-----
  python nod_fmriprep_qc_movie.py [--tasks coco imagenet] [--max_sessions 8]
                                   [--subs sub-01 sub-02 ...]

Output
------
  OUT_DIR / NaturalObjectDataset_<subs>_qc_small.mp4
"""
...
# ---------------------------------------------------------------------------
DATASET_NAME = "NaturalObjectDataset"
FMRIPREP = Path(os.getenv("DATASETS_ROOT")) / "NaturalObjectDataset/derivatives/fmriprep"
OUT_DIR  = Path(os.getenv("PROJECT_ROOT")) / "src/fmriDatasetPreparation/visualizations/output/qc"

DEFAULT_TASKS = ['coco', 'imagenet']   # main stimulus-driven tasks; task-floc
                               # (localizer) and task-prf (mapping) are excluded by default
...
# NOD keeps anatomy in a dedicated ses-anat session (no func/), but its anat
# filenames carry an extra "run-01" entity not present in most other
# datasets (e.g. sub-01_ses-anat_run-01_desc-preproc_T1w.nii.gz). Anat files
# are located by globbing on distinctive suffixes rather than exact
# filenames so this (and any other dataset's filename quirks) doesn't need
# special-casing. Sessions/tasks/runs are discovered dynamically by globbing.
...
def make_run_summary_fig(sub, sessions, tasks):
    ...
    cmap = matplotlib.colormaps["tab20"].resampled(len(sessions))
    ...
    # stays within the canvas width for any session-name style, and stacked
    # in rows (reserved via measured height) instead of a single
    # axes-anchored row — a wide single-row legend previously forced
    # tight_layout to squeeze the plot axes down to a sliver to keep the
    # legend on-canvas.
