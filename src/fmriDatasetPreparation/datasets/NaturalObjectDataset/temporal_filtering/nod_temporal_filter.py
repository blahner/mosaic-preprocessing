...
def main(args):
    if args.task not in ['imagenet', 'coco', 'floc', 'prf']:
        raise ValueError(f"task not recognized. You entered {args.task} but it must be one of 'imagenet', 'coco', 'floc', 'prf'")

    #define subject and session
    subject = f"sub-{int(args.subject):02}"
    session_folders = glob.glob(os.path.join(args.dataset_root, "derivatives","fmriprep",subject, f"ses-{args.task}*"))
    ...
    for session_path in session_folders:
        ...
        for r in range(1, numruns+1): #each run is temporally filtered independently
            if args.task in ['imagenet', 'coco', 'floc', 'prf']: #for legacy with other datasets that might not zfill their run index
                run = f"{r:02}"
...
if __name__ == '__main__':
    ...
    parser.add_argument("-d", "--dataset_root", default="/data/vision/oliva/scratch/datasets/NaturalObjectDataset", help="The root path to the dataset directory")
