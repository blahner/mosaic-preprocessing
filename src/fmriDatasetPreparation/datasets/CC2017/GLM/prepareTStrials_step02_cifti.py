...
sub_save_root = os.path.join(args.root, "TSTrialEstimates", subject, "estimates-prepared", "prepared_allvoxel_pkl")
if not os.path.exists(sub_save_root):
    os.makedirs(sub_save_root)
step_save_root = os.path.join(args.root, "TSTrialEstimates", subject, "estimates-prepared", "step02")
...
if args.verbose:
    print("loading non-zscored estimates for noiseceiling calculation")
#load non-zscored estimates for noisceiling calculation
#compute noise ceiling on non-zscored estimates
with open(os.path.join(args.root, "TSTrialEstimates", subject, "estimates-prepared", "step01", f"{subject}_z=0_TSTrialEstimates_task-test.pkl"), 'rb') as f:
    estimates_noz_test, condition_order_test = pickle.load(f)

if args.verbose:
    print("Aggregating repeated conditions")
numvertices = estimates_noz_test.shape[1]
numreps = 10
repeated_conditions = list_rep(condition_order_test, 10) #just get the conditions repeated exactly numpreps times
estimates_noz_test_matrix = np.zeros((numvertices, numreps, len(repeated_conditions)))
for count, cond in enumerate(repeated_conditions):
    idx = [i for i, c in enumerate(condition_order_test) if c == cond] #all indices of repeated condition "cond"
...
ncsnr_test, noiseceiling_test = calculate_noiseceiling(estimates_noz_test_matrix)
if args.verbose:
...
del estimates_noz_test, estimates_noz_test_matrix #free RAM

if args.verbose:
    print("Loading zscored betas")
#load zscored estimates
with open(os.path.join(args.root, "TSTrialEstimates", subject, "estimates-prepared", "step01", f"{subject}_{z_string}_TSTrialEstimates_task-train.pkl"), 'rb') as f:
    estimates_matrix_train, condition_order_train = pickle.load(f)
with open(os.path.join(args.root, "TSTrialEstimates", subject, "estimates-prepared", "step01", f"{subject}_{z_string}_TSTrialEstimates_task-test.pkl"), 'rb') as f:
