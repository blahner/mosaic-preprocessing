...
for count, segment in enumerate(segments):
    for rep in range(1,numrepeats+1):
        ...
        num_vertices = fmri_interp.shape[0]  # get shape of data for convenience
        ...
        for count, r in enumerate(range(0, num_regressors_of_interest)):
            if (subject == 'subject3') and (segment == 'test2') and (rep == 9):
                estimates[r,:] = fmri_interp[:,int(r*(videoChunk/TR_resamp)+BOLDoffset/TR_resamp+10/TR_resamp)]
            else:
                estimates[r,:] = fmri_interp[:,int(r*(videoChunk/TR_resamp) + BOLDoffset/TR_resamp)]
        ...
        if args.verbose:
            print("saving design matrix and single trial estimates")
