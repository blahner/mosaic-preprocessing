...
sub_save_root = os.path.join(args.root, "GLMsingleAllRuns", subject)
...
for count, segment in enumerate(segments):
    ...
    for rep in range(1,numrepeats+1):
        ...
        if segment in ['test2']: #246 volumes instead of the usual 245
            dummy_offset=0#10 #this overwrites the above dummy_offset default
        elif segment in ['test3','test4','test5']:
            dummy_offset=0
        ...
        if (subject == 'subject3') and (segment == 'test2') and (rep == 9):
            fmri_interp = fmri_interp[:,5:] #make all test runs the same cut of fmri starting from 5 seconds into the video to the end.
            
        numscans_interp = fmri_interp.shape[1]
        data.append(fmri_interp)

        #load events
        if (subject == 'subject3') and (segment in ['test2']):
        ...
        for count, r in enumerate(range(0, num_regressors_of_interest)):
            idx = int(count*(videoChunk/TR_resamp))
            #if (subject == 'subject3') and (segment == 'test2') and (rep == 9):
            #    run_design[int(r*(videoChunk/TR_resamp)+(10/TR_resamp)), r] = 1 
            #else:
            #    run_design[idx, count] = 1
            run_design[idx, count] = 1
    ...
    outputdir_glmsingle = os.path.join(sub_save_root, f"{args.task}", f"{segment}")
    ...
    np.save(join(outputdir_glmsingle, f"{subject}_{segment}_stimOrder.npy"), stim_order, allow_pickle=True)
