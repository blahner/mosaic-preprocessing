...
for subj in {1..4}; do
    ...
    uv run --project "${PROJECT_ROOT}" python ${ROOT}/temporal_filter.py -s ${subj} -t '5000scenes'
    uv run --project "${PROJECT_ROOT}" python ${ROOT}/temporal_filter.py -s ${subj} -t 'localizer'
