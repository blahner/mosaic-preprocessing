...
def main(args):
    if args.task not in ['action']:
        raise ValueError(f"task not recognized. You entered {args.task} but it must be one of 'action'")

    #define subject and session
    subject = f"sub-{int(args.subject):02}"
    session = "ses-action01" #HAD only has one session per subject
...
if __name__ == '__main__':
    ...
    parser.add_argument("-t", "--task", type=str, required=True, help="What task (action) to analyze)")
    parser.add_argument("-d", "--dataset_root", default="/data/vision/oliva/scratch/datasets/HumanActionsDataset", help="The root path to the dataset directory")
