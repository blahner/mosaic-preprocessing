...
def main(args):
    ...
    session = f"ses-nsd{int(args.session):02}"
    if args.verbose:
    ...
    numruns = len(glob.glob(os.path.join(args.dataset_root, "derivatives", "fmriprep", subject, session, "func", f"{subject}_{session}_task-nsdcore_run-*_desc-confounds_timeseries.tsv")))  #nothing special about the confounds file choice. should be 12 runs
    if args.verbose:
    ...
    for count, run in enumerate(range(1,numruns+1)):
        if args.verbose:
            print("run:",run)
        #load data
        fmri_img = nib.load(os.path.join(args.dataset_root, "derivatives", "fmriprep", subject, session, "func", f"{subject}_{session}_task-nsdcore_run-{run:02}_space-fsLR_den-91k_bold.dtseries.nii")).get_fdata()
        ...
        tmp = pd.read_table(os.path.join(args.dataset_root, "Nifti", subject, session, "func", f"{subject}_{session}_task-nsdcore_run-{run:02}_events.tsv"))
        for idx, stim_id in enumerate(tmp.loc[:,'73k_id']):
            stimIDX = f"73k_id-{stim_id}"
    ...
    outputdir_glmsingle = os.path.join(args.dataset_root, "derivatives", "GLM", subject, f"{session}")
    ...
    with open(os.path.join(outputdir_glmsingle, f"{subject}_{session}_task-nsdcore_conditionOrderDM.pkl"), 'wb') as f:
