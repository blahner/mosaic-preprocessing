...
for subj in {01..08}; do
    ...
    docker run \
        ...
        --bids-filter-file /data/nsdsynthetic_bids_filter_file.json \
    ...
    shopt -s nullglob
    subj_wf_dirs=("${WF_ROOT}"/sub_${subj}_*wf)
    shopt -u nullglob
    if [ ${#subj_wf_dirs[@]} -eq 0 ]; then
        echo "WARNING: no work dir matching ${WF_ROOT}/sub_${subj}_*wf - nothing deleted"
    else
        rm -r "${subj_wf_dirs[@]}"
    fi
done
