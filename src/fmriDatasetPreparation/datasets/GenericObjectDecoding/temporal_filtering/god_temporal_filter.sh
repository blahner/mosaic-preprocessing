...
for subj in {01..05}; do
    ...
    uv run --project "${PROJECT_ROOT}" python ${ROOT}/god_temporal_filter.py -s ${subj} -t 'imagery'
    uv run --project "${PROJECT_ROOT}" python ${ROOT}/god_temporal_filter.py -s ${subj} -t 'perception'
