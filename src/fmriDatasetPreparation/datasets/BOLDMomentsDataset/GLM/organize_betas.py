...
def main(args):
    ...
    tmp = pd.read_table(os.path.join(args.dataset_root, "derivatives", "stimuli_metadata", "bmd_stiminfo.tsv"),index_col=False)
    ...
    assert(len(test_stimuli) == 102)
    assert(len(train_stimuli) == 1000)
    ...
    for ses in range(2,6):
        ...
        for event in events_run:
            xxxx_stimulus_names = event['trial_type']
            for xxxx_stim in xxxx_stimulus_names:
                mit_name = annotations[f"{xxxx_stim.split('vid')[-1]:04}"]['MiT_filename'].split('/')[-1]
                ses_conds.append(mit_name)#the order the stimuli were presented
        ...
        for stim in ses_conds:
            if stim in train_stimuli:
                betas_tmp_train[stim].append(fmri_data_wb_train_normalized[train_count, :]) #collects all z-scored responses per stimulus
                train_count += 1
            elif stim in test_stimuli:
                betas_tmp_test[stim].append(fmri_data_wb_test_normalized[test_count, :]) #collects all normalized responses per stimulus
                test_count += 1
