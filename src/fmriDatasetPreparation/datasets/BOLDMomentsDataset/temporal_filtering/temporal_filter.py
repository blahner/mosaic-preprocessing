...
def main(args):
    ...
    if args.task in ['test','train']:
        sessions = [2,3,4,5]
    elif args.task in ['rest','localizer']:
        sessions = [1]
    else:
        raise ValueError("task not recognized. Must be either train, test, localizer, or rest")
    ...
    for s in sessions:
        ...
        fmri_output_path = os.path.join(args.dataset_root, "derivatives", "versionC", "temporal_filtering", subject, session)
        if not os.path.exists(fmri_output_path ):
            os.makedirs(fmri_output_path )

        numruns = len(glob.glob(os.path.join(args.dataset_root, "derivatives","versionC","fmriprep",subject, session, "func", f"{subject}_{session}_task-{args.task}_run-*_desc-confounds_timeseries.tsv")))
        assert(numruns > 0)
        ...
        for run in range(1, numruns+1): #each run is temporally filtered independently
            ...
            confounds = pd.read_table(os.path.join(args.dataset_root, "derivatives", "versionC", "fmriprep", subject, session, "func", f"{subject}_{session}_task-{args.task}_run-{run}_desc-confounds_timeseries.tsv"))
            ...
            func_root = os.path.join(args.dataset_root, 'derivatives', "versionC", 'fmriprep', subject, session, 'func')
...
if __name__ == '__main__':
    ...
    parser.add_argument("-t", "--task", type=str, required=True, help="What task (test or train or rest or localizer) to analyze)")
