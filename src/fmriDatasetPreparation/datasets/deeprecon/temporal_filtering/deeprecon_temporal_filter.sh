...
for subj in {01..03}; do
    ...
    uv run --project "${PROJECT_ROOT}" python ${ROOT}/deeprecon_temporal_filter.py -s ${subj} -t 'imagery'
    uv run --project "${PROJECT_ROOT}" python ${ROOT}/deeprecon_temporal_filter.py -s ${subj} -t 'perception'
