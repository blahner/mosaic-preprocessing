...
def main(args):
    ...
    filenames = glob.glob(os.path.join(args.root, "betas_fsLR32k", "*_nanfilled-*.npz")) #note that order is not preserved
    ...
    with h5py.File(os.path.join(args.root,'mosaic_version-1_0_0.hdf5'), 'a') as f:
        for group in tqdm(groups, total=len(groups), desc="Adding 'sub-XX_DATASET' groups to hdf5 file"):
            ...
            for response_filename in group_response_filenames:
                #"sub-01_BMD_stimulus-0-2-4-4-9-1-5-4-6102449154_phase-test_rep-0_nanfilled-adjacency.npz"
                ...
                dset.attrs.create('presented_stimulus_filename', presented_stimulus_filename)
                dset.attrs.create('image_stimulus_filename', image_stimulus_filename)
